% frmcs_appsession_fr.tex  — Draft v2.4
...
\begin{titlepage}
  \vspace*{3cm}{\sffamily\color{etsiblue}
    ...
    {\large Contribution de travail -- Draft v2.4}\\[0.5em]
    ...
    {\small\textbf{Documents cibles :} ETSI TS 103\,764 (RT-0095),
      ...
      FIS-7970, FRS FU-7120, AT-7800, TOBA FRS-7510)}}
\end{titlepage}
...
Toutes les occurrences internes « session » $\to$ « session de service » dans les TS.
...
\begin{enumerate}[leftmargin=2em]
  \item Si \texttt{remoteId} se termine par \texttt{.ertms} (format ETCS
    \texttt{id.ty.cc.ertms}, UIC AT-7800) : complétion par le domaine courant
...
\begin{notebox}
  ...
  sans impact sur \OBAPP, \TSAPP, SIP ni les specs UIC (ERJU/OCORA).
\end{notebox}
...
\begin{enumerate}[leftmargin=2em]
  \item \textbf{Initialisation} : POST à l'ASAS ; reçoit \appterm, clé Ed25519,
    liste d'alias, politique multipath, période keepalive.
  \item \textbf{Sessions de service additionnelles} : établies selon liste d'alias +
    FROP ; chacune déclarée à l'ASAS avec \texttt{sessionToken} signé.
  ...
  \item \textbf{Libération} : DELETE à l'ASAS + libération de chaque session de
    service selon son propre protocole.
\end{enumerate}
...
\begin{longtable}{>{\bfseries}p{3.8cm} p{10.5cm}}
  ...
  AIR & Fonction interne résolvant \texttt{remoteId} en \textbf{identifiant de
    ...
    sinon pass-through FA MCx. Point d'extension (DNS/FROP, ERJU/OCORA). \\[0.3em]
  ASM & Fonction interne gérant le cycle de vie des sessions et sessions de
    service, pilotant les interactions ASAS. \\[0.3em]
  ASAS & Serveur REST/TLS sur l'IP post-NAT de la GW (SD\_PUB\_IP).
    Ancrage de confiance : première session de service MCx. Clés Ed25519 suffisent.
    Absorbe \texttt{FUTURE\_CRITICAL\_SERVICE} sans modification. \\[0.3em]
...
\subsection{Modèle de confiance et authentification}
\label{sec:trust}

\textbf{La première session de service MCx est un service sûr déjà en charge
d'identifier les parties} -- ancrage de confiance suffisant.

\subsubsection*{Niveau 1 -- Confidentialité du canal (TLS 1.3)}

Certificat serveur \textbf{auto-signé} possible -- aucune PKI inter-opérateur.

\begin{notebox}
  \textbf{Note FSNNI multi-hop} : le topology hiding ne compromet pas la confiance --
  le \texttt{sessionToken} Ed25519 protège l'intégrité par message indépendamment
  du chemin réseau. Seule contrainte : routabilité ASAS sur SD\_PUB\_IP
  (§\ref{sec:nat}).
\end{notebox}

\subsubsection*{Niveau 2 -- Ancrage de confiance}

Le MC Service Server a authentifié la GW appelante. Échange Ed25519 = bootstrapping sûr.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{$N = 2$} échecs consécutifs (fixe) \textrightarrow\ \texttt{DEGRADED}.
...
\begin{longtable}{p{5.5cm} p{4cm} p{4.5cm}}
  ...
  Multipath interne & Multipath session & \textbf{Multipath service session} \\
...
\paragraph{X.2 -- Transport et modèle de confiance}

\textbf{Transport :} TLS 1.3, HTTP/2 + JSON. Port : FROP.
\textbf{Certificat serveur :} Auto-signé ou CA locale.
\textbf{Adresse IP ASAS :} IP source post-NAT SIP (SD\_PUB\_IP, RT-0097 §6.2.2.4).
\textbf{Modèle de confiance :} Ancrage MCx + clés Ed25519. Pas de PKI inter-GW.
...
\paragraph{X.9 -- Mapping erreurs \textrightarrow\ causes \OBAPP}
...
\begin{longtable}{p{5cm} p{6.5cm} p{2cm}}
  ...
  \textbf{ETSI TS 103\,764} & Session + service session + AIR + ASAS +
    service identifier (§3.1) + ASM + \textbf{Annexe X} & \\
  \textbf{ETSI TS 103\,765-3} & Remplacement + Step 1 (AIR→id de service + SD\_PUB\_IP) & inchangées \\
  \textbf{ETSI TS 103\,765-4} & Remplacement + Step 0 + alias list (déclenchement immédiat) & inchangées \\
  \textbf{ETSI TS 103\,765-5} & Remplacement + binding Multipath & inchangées \\
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item[\st{2.}] Confiance multi-hop FSNNI -- \textit{Résolu} (§\ref{sec:trust}).
  \item[\st{3.}] Sérialisation sessionToken -- \textit{Résolu}.
  \item[\st{4.}] Règles AIR -- \textit{Résolu}.
  \item[\st{5.}] Timer de recovery -- \textit{Résolu} : INTEGER(1..180)\,s ; $\leq 5$\,s pour ETCS/ATP.
  \item[\st{6.}] T\_KA / N -- \textit{Résolu} : INTEGER(1..10)\,s ; $N=2$ ; T\_KA $\leq$ timer/2.
