% frmcs_appsession_en.tex  — Draft v2.4
...
% Subscript shorthand for interface names
...
\begin{titlepage}
  \vspace*{3cm}{\sffamily\color{etsiblue}
    ...
    {\large Working Contribution -- Draft v2.4}\\[0.5em]
    ...
    {\small\textbf{Target documents:} ETSI TS 103\,764 (RT-0095), TS 103\,765-3 (RT-0097),
      ...
      FRS FU-7120, AT-7800, TOBA FRS-7510)}}
\end{titlepage}
...
\begin{enumerate}[leftmargin=2em]
  \item If \texttt{remoteId} ends with \texttt{.ertms} (ETCS format
    \texttt{id.ty.cc.ertms}, UIC AT-7800): append the current MCx domain
    \textrightarrow\ \texttt{id.ty.cc.ertms@current\_domain}.
...
\begin{notebox}
  ...
  \TSAPP, SIP, or UIC specifications (ERJU/OCORA).
\end{notebox}
...
\begin{enumerate}[leftmargin=2em]
  \item \textbf{Initialisation}: POST to ASAS with \texttt{svcSessionId\_1} as
    correlator; receives \appterm, Ed25519 public key, alias list, multipath policy,
    keepalive period.
  \item \textbf{Additional service sessions}: established per alias list + FROP; each
    declared to ASAS with signed \texttt{sessionToken}.
  ...
  \item \textbf{Release}: DELETE to ASAS + release each service session per its protocol.
\end{enumerate}
...
\begin{longtable}{>{\bfseries}p{3.8cm} p{10.5cm}}
  ...
  AIR & Internal Gateway function resolving \texttt{remoteId} to a service identifier
    ...
    \texttt{*.ertms} \textrightarrow\ \texttt{*.ertms@current\_domain};
    otherwise pass-through FA MCx. Extension point (DNS/FROP, ERJU/OCORA).
    Not visible to applications. \\[0.3em]
  ASM & Internal Gateway function managing session and service session lifecycle,
    driving ASAS interactions. Not visible to applications. \\[0.3em]
  ASAS & REST/TLS API server on the Gateway's post-NAT IP (SD\_PUB\_IP).
    Trust anchored by first MCx service session. Ed25519 key exchange sufficient --
    no inter-GW PKI required. Absorbs \texttt{FUTURE\_CRITICAL\_SERVICE} without
    modification. \\[0.3em]
...
\subsection{Trust Model and Authentication}
\label{sec:trust}

\textbf{The first MCx service session is a trusted service already responsible for
identifying the parties} -- its establishment is a sufficient trust anchor.

\subsubsection*{Level 1 -- Channel Confidentiality (TLS 1.3)}

Server certificate may be \textbf{self-signed} -- no inter-operator PKI required.

\begin{notebox}
  \textbf{FSNNI topology hiding note}: a potential intermediate domain performing
  topology hiding does not compromise trust -- the Ed25519 \texttt{sessionToken}
  provides per-message integrity regardless of the network path. The only constraint
  is ASAS reachability on SD\_PUB\_IP (covered by §\ref{sec:nat}).
\end{notebox}

\subsubsection*{Level 2 -- Trust Anchor (First MCx Service Session)}

The MC Service Server authenticated the calling Gateway before accepting the session.
Ed25519 public key exchange over this channel bootstraps trust.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{$N = 2$} consecutive failures (fixed) \textrightarrow\ \texttt{DEGRADED}.
...
\begin{longtable}{p{5.5cm} p{4cm} p{4.5cm}}
  ...
  Internal Multipath procedure & Multipath session & \textbf{Multipath service session} \\
...
\paragraph{X.2 -- Transport and Trust Model}

\textbf{Transport:} TLS 1.3, HTTP/2 + JSON. Port: FROP.

\textbf{Server certificate:} Self-signed or any locally-deployed CA.
...
\textbf{Trust model:} First MCx service session anchor + Ed25519 keys.
No inter-GW PKI required.
...
{ "appSessionId_call": "uuid-v4", "svcSessionRef_1": "uuid-v4",
  ...
  "keepaliveReq": 2,
  "mpCapabilities": { "supportedPolicies": ["DUPLICATION","RECOVERY"], "maxPaths": 4 } }

// Response 200 OK
{ "appSessionId_term": "uuid-v4", "svcSessionRef_1": "uuid-v4",
  "terminatingPublicKey": "base64(Ed25519)", "keepaliveAccepted": 2,
  "mpPolicy": { "policy": "DUPLICATION|RECOVERY|LOADBALANCE|NONE", "activePaths": 2 },
...
\begin{longtable}{p{5cm} p{6.5cm} p{2cm}}
  ...
  \textbf{ETSI TS 103\,764} & Session + service session + AIR + ASAS + service identifier
    (§3.1) + ASM + \textbf{Annex X} & \\
  \textbf{ETSI TS 103\,765-3} & Systematic replacement + Step 1 (AIR→service id + SD\_PUB\_IP) & unchanged \\
  \textbf{ETSI TS 103\,765-4} & Systematic replacement + Step 0 + immediate alias list mgmt & unchanged \\
  \textbf{ETSI TS 103\,765-5} & Systematic replacement + Multipath binding & unchanged \\
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item[\st{4.}] AIR rules -- \textit{Resolved}: \texttt{*.ertms}
    \textrightarrow\ \texttt{@current\_domain}; otherwise pass-through.
  \item[\st{5.}] Recovery timer -- \textit{Resolved}: INTEGER(1..180)\,s; $\leq 5$\,s for ETCS/ATP.
  \item[\st{6.}] T\_KA / N -- \textit{Resolved}: INTEGER(1..10)\,s; $N=2$; T\_KA $\leq$ timer/2.
